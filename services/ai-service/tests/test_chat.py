...
def test_chat_only_sends_four_recent_raw_messages_to_model() -> None:
    model = CapturingChatModel()
    ...
    assert "tin cũ 1" not in raw_contents
    assert "trả lời cũ 1" not in raw_contents
    assert raw_contents[-5:] == [
        "tin cũ 2",
        "trả lời cũ 2",
        "tin mới",
        "trả lời mới",
